...
def tranform_to_color(input):
    b,h,w = input.shape

    map = np.zeros((input.shape[0], input.shape[1], input.shape[2], 3), dtype=np.uint8)

    # color1  red, green, yellow
    colors1 = [[0,0,0], [0,0,255],[0,255,0], [0,255,255]]

    # color2  blue, pink, cyan
    colors2 = [[0,0,0], [255,0,0],[255,0,255], [255,255,0]]

    for j in range(b):
        for i, rgb in zip(range(4), colors1):
            (map[j])[input[j] == i] = rgb
